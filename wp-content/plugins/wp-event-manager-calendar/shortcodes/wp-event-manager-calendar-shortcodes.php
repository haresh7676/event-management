...
class WP_Event_Manager_Calendar_Shortcodes {
	...
	public  function events_calendar($atts) 
	 {  
	       ...
	       extract( shortcode_atts( apply_filters('event_manager_event_calender_default', array('month'  => $month, 'year' => $year) ), $atts ));	       
	       ...
	       if(isset($_REQUEST['events_calendar_nonce']) && wp_verify_nonce($_REQUEST['events_calendar_nonce'], 'events_calendar_nonce')) 
	       {
    		    ...
    		    elseif( isset( $_REQUEST['calendar_navigation'] ) && ( $_REQUEST['calendar_navigation'] == 'calendar_navigation_next')) 
    		    {
		          ...
		          wp_send_json( WP_Event_Manager_Calendar_Shortcodes::events_calendar_data('events-calendar.php', $month,$year,FALSE) );
    		}
    		else
    		{ 
    		   return WP_Event_Manager_Calendar_Shortcodes::events_calendar_data('events-calendar.php', $month,$year,FALSE);
    		}
	 ...
	 */   
	 ...
	 {  
        ...
        public function events_calendar_data($template,$month,$year,$isWidget )
        {
                $time  = current_time('timestamp');
                $today = date('j', $time);
		...
		{
		   $message= get_event_manager_template_part( 'content', 'no-events-found' );		  
		}
