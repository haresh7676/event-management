...
class WP_Event_Manager_Calendar extends GAM_Updater {
	...
	public function __construct() 
	{
		...
		***/
		// Define constants
		define( 'EVENT_MANAGER_CALENDAR_VERSION', '1.4.2' );
		...
		// Activate
		register_activation_hook(  __FILE__ , array( $this ,'wp_event_manager_calendar_activate_plugin'));
		
		//Deactivate
		register_deactivation_hook( __FILE__, array( $this ,'wp_event_manager_calendar_deactivate_plugin'));		
	...
	/**
	 * Add Default event Calendar page at the time of plugin activation
	 */
	function wp_event_manager_calendar_activate_plugin() {
	    // Create post object
	    $calendar_page = array(
	        'post_title'    => wp_strip_all_tags( 'Events Calendar' ),
	    ...
	    $calendar_page_id = wp_insert_post( $calendar_page );
	    update_option( 'event_manager_calendar_page_id', $calendar_page_id, true );
	    
	}
	/**
	 * Delete event calendar page on plugin deactivation time
	 */
	function wp_event_manager_calendar_deactivate_plugin() {
	    
	    //get Id of calendar page
	    $page_id = get_option('event_manager_calendar_page_id', true);
	    // Delete the page from the database
	    wp_delete_post($page_id);
	    delete_option('event_manager_calendar_page_id');
	}
	...
	public function load_plugin_textdomain() {
		...
		load_textdomain( $domain, WP_LANG_DIR . "/wp-event-calendar/".$domain."-" .$locale. ".mo" );
