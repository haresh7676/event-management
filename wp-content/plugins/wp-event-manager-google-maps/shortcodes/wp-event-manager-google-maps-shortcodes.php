...
private $_post_stack = array();
	...
	public function __construct() {	
		...
		add_action( 'single_event_listing_meta_end',		array( $this, 'single_event_maps' ), 10, 4);	
		...
		foreach($temp_locations as $event_locations){
			...
			$formatted_address = $event_locations['formatted_address'];
					...
					$event_tool_tip =  $event_locations[0]['tool_tip'];
							array_push($temp_location ,$formatted_address,$lat,$long,$event_tool_tip);
							array_push($all_locations,$temp_location);
				}
				...
				$all_locations[] =array($address,$latitude,$longitude);
		}
						?>
		       ...
		       console.log(locations);
		        for (i = 0; i < locations.length; i++) {  
		          var marker = new google.maps.Marker({
		            ...
		            map: map,
		        	icon : <?php echo json_encode($marker);?>
		          });
			...
			jQuery(function(){
				...
				*/
				...
				for (i = 0; i < locations.length; i++) { 
				  ...
				  google.maps.event.addListener(marker, 'click', (function(marker, i) {
					return function() {
					  infowindow.setContent(locations[i]['tool_tip']);
					  infowindow.open(map, marker);
					}
				  })(marker, i));
				}
