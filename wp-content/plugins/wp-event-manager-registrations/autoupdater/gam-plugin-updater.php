...
class GAM_Updater {
	...
	public function activate_licence( $licence_key, $email ) {
		try {
			...
			$activate_results = json_decode( GAM_Updater_Key_API::activate( array(
				'email'          => $email,
	...
	public function deactivate_licence() {
		$reset = GAM_Updater_Key_API::deactivate( array(
				'api_product_id' => $this->plugin_slug,
	...
	public function get_plugin_version() {
		$response = GAM_Updater_API::plugin_update_check( array(
	...
	public function get_plugin_info() {
		$response = GAM_Updater_API::plugin_information( array(
			'plugin_name'    => $this->plugin_name,
			'version'        => $this->plugin_data['Version'],
			'api_product_id' => $this->plugin_slug,
			'licence_key'    => $this->api_key,
	...
	public function handle_errors( $errors ) {
		if ( ! empty( $errors['no_key'] ) ) {
			$this->add_error( sprintf( __('A licence key for %1$s could not be found. Maybe you forgot to enter a licence key when setting up %2$s.', 'wp-event-manager-registrations'), esc_html( $this->plugin_data['Name'] ), esc_html( $this->plugin_data['Name'] ) ) );
