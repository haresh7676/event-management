...
<div id="wpmpResetPasswordSection" class=" <?php echo empty($is_url_has_token) ? ' hidden' : 'ds' ?>">   
    ...
    <form name="wpmpResetPasswordForm" id="wpmpResetPasswordForm" method="post">
        ...
        ?>
        <div class="buttonwpr centerbtn"><button type="submit" class="btn defulat-custom-btn"><?php _e($resetpassword_button_text, $this->plugin_name); ?></button></div>
        <div class="or-dvd-line phone-or-reset"><span>or</span></div>
        <div class="buttonwpr center">
            <a href="#" class="btn quirk-btn defulat-custom-btn quirk-resetbtn btnQuirktasticReset">Quirktastic</a>
        <button type="button" id="btnReturnToLogin" class="btn defulat-custom-btn btnReturnToLogin"><?php _e($returntologin_button_text, $this->plugin_name); ?></button>
        </div>

    </form>
</div>
<div id="wpmpQuirktasticResetPasswordSection" class="hidden <?php //echo empty($is_url_has_token) ? ' hidden' : 'ds' ?>">
    <?php
    $wpmp_form_settings = get_option('wpmp_form_settings');
    ...
    <div id="wpmp-resetpassword-quirk-loader-info" class="wpmp-loader" style="display:none;">
        <img src="<?php echo plugins_url('images/ajax-loader.gif', dirname(__FILE__)); ?>"/>
        <span><?php _e('Please wait ...', $this->plugin_name); ?></span>
    </div>
    <div id="wpmp-resetpassword-quirk-alert" class="alert alert-danger" role="alert" style="display:none;"></div>

    <form name="wpmpQuirktasticResetPasswordForm" id="wpmpQuirktasticResetPasswordForm" method="post">
        <?php
        // check if the url has token
        if (!$is_url_has_token) :
        ...
        <div class="or-dvd-line phone-or-reset"><span>or</span></div>
        <div class="buttonwpr center">
            <a href="#" class="btn quirk-btn defulat-custom-btn quirk-resetbtn btnEmailReset">Reset With Email</a>
            <button type="button" id="btnReturnToLogin" class="btn defulat-custom-btn btnReturnToLogin"><?php _e($returntologin_button_text, $this->plugin_name); ?></button>
        </div>
